...
def get_RSRDE_regrets(*, dist_type: str, num_training_bids: int, num_training_rounds: int,
                true_dist_cdf: Callable, ideal_revenue: float, test_bids: list, output_uri: str, regrets_uri: str):
    # load training results
    training_results = pickle_load(os.path.join(output_uri, dist_type, "trains.pkl"))
...
def eval(*, dist_type: str, mechanism: str, num_training_bids: int | None = None, num_training_rounds: int | None = None, output_uri: str):
    tests = pickle_load(os.path.join(output_uri, dist_type, "tests.pkl"))
    regrets_uri = get_regrets_uri(output_uri=output_uri, dist_type=dist_type, mechanism=mechanism, num_training_bids=num_training_bids, num_training_rounds=num_training_rounds)
...
def parse_args():
    ...
    parser.add_argument("--mechanism", required=True, help="Specify the mechanism", choices=["DOP", "RSOP", "RSKDE", "RSRDE"])
    parser.add_argument("--num-training-bids", choices=[100, 1000], type=int)
    parser.add_argument("--num-training-rounds", choices=[20], type=int)
    ...
    if args.mechanism == "RSRDE":
        if not args.num_training_bids or not args.num_training_rounds:
            raise ValueError("Mechanism RSRDE must specify --num-training-bids and --num-training-rounds from choices.")
    return args
...
def main():
    ...
    eval(dist_type=dist_type, mechanism=mechanism, num_training_bids=num_training_bids, num_training_rounds=num_training_rounds, output_uri=args.output_uri)
